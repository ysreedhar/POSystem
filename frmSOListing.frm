...
Begin VB.Form frmSOListing 
   ...
   Begin VB.CheckBox chkShowCancelled 
      Caption         =   "Show Cancelled Service Orders"
      Height          =   495
      Left            =   480
      TabIndex        =   4
      Top             =   600
   ...
   End
End
Attribute VB_Name = "frmSOListing"
...
fldata3.Open "select * from SO_Header", cn, 3, 2
Else
fldata3.Open "select * from SO_Header where status <> 6", cn, 3, 2
End If
With flxPurchaseOrders
.Rows = 1
...
fldata3.Open "select * from SO_Details where POID =" & intPOID, cn, 3, 2
With flxPODetails
.Rows = 1
    While Not fldata3.EOF
        .Rows = .Rows + 1
 ...
 End With
 If fldata3.State Then fldata3.Close
If dbltotal > 0 Then lblPOtotal.Caption = "Total Value of Service Order = " & flxPurchaseOrders.TextMatrix(flxPurchaseOrders.Row, 11) & "." & Format(dbltotal, "###,###,##0.00") Else lblPOtotal.Caption = ""
End Sub
