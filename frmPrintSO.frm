...
Begin VB.Form frmPrintSO 
   ...
   Begin VB.CheckBox chkShowPrinted 
      Caption         =   "Show Printed Service Orders"
      Height          =   495
      Left            =   240
      TabIndex        =   3
      Top             =   120
...
strCommand = "Select distinct(PO_OrderNo) from SO_Header where status in (1,2) order by PO_orderNo"
Else
strCommand = "Select distinct(PO_OrderNo) from SO_Header where status = 1 order by po_OrderNo asc"
End If
...
strSQL = "SHAPE {SELECT * FROM vSOHeader where PO_OrderNo = '" & dblPOnumber & "' } APPEND " _
         & "({SELECT * FROM vSODetails_report where POID = (select poID from vSOHeader where PO_OrderNo = '" & dblPOnumber & "' )} AS rsPOItems " _
         & "RELATE POID TO POID)"
